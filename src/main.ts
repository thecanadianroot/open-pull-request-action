...
async function run(): Promise<void> {
    try {
        const token: string | undefined = core.getInput('token', {required: false}) || process.env.TOKEN;
        const base: string | undefined = core.getInput('base', {required: false}) || process.env.BASE;
        const head: string | undefined = core.getInput('head', {required: false}) || process.env.HEAD;
        const sha: string | undefined = core.getInput('sha', {required: false}) || process.env.SHA;
        const title: string | undefined = core.getInput('title', {required: false}) || process.env.TITLE;
        ...
        if (!token || !base || !head || !title) {
            ...
            core.info(`'TOKEN', 'BASE', 'HEAD' and 'TITLE' environment variables can also be used.`);
            return;
        }
